...
public List<Inventory> searchInventories(String productName, String shopName, Integer minQuantity, Integer maxQuantity) {
    List<Inventory> list = new ArrayList<>();
    StringBuilder sql = new StringBuilder(
        "SELECT i.InventoryID, i.ProductID, p.ProductName, i.ShopID, s.ShopName, i.Quantity, i.LastUpdated "
      + "FROM Inventory i "
      + "JOIN Product p ON i.ProductID = p.ProductID "
      + "LEFT JOIN Shop s ON i.ShopID = s.ShopID WHERE 1=1 ");

    List<Object> params = new ArrayList<>();

    if (productName != null && !productName.trim().isEmpty()) {
        sql.append(" AND p.ProductName COLLATE Latin1_General_CI_AI LIKE ? ");
