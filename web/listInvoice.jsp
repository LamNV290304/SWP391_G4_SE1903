            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="post" action="InvoiceServlet">
                        ...
                        <div class="mb-3">
                            <label for="searchInvoiceID" class="form-label">Tìm theo Mã HĐ hoặc Khách Hàng:</label>
                    ...
                    <c:forEach var="inv" items="${invoiceList}">
                        <tr>
                            ...
                            <form method="post" action="InvoiceServlet" style="margin:0;">
                                ...
                                <input type="hidden" name="employeeID" value="${inv.employeeID}" /> 
                                <td>${inv.invoiceID}</td>
                                <td>
...
<!--                                <td>
                                    <select class="form-select" name="employeeID" required>
                                        <c:forEach var="employee" items="${employees}"> <%-- 'employees' vẫn cần được truyền từ Servlet --%>
                                            <option value="${employee.id}" <c:if test="${inv.employeeID == employee.id}">selected</c:if>>
                                                ${employee.fullname}
                                            </option>
                                        </c:forEach>
                                    </select>
                                ...
                                <td>
                                    ...
                                    <select class="form-select" name="shopID" required>
                                        <c:forEach var="shop" items="${allShops}"> <%-- Cần đảm bảo "allShops" được truyền từ Servlet --%>
                                            <option value="${shop.shopID}" <c:if test="${inv.shopID == shop.shopID}">selected</c:if>>
                                                ${shop.shopName}
                                            </option>
                                        </c:forEach>
                                    </select>
